...
export default function connectAdvanced(
  ...
  */
  ...
  /** 
   * connect(mapState, mapDispatch)(AppLoading)
   * wrapWithConnect  = connect(mapState, mapDispatch)
   * WrappedComponent = AppLoading;
   * 返回的是封装的组件。WrappedComponent既包装的react组件
  */
  return function wrapWithConnect(WrappedComponent) {
    ...
    // 生成派生类的props seletor connect的props
    // 柯里化函数，闭包（在下面的类中这些值可以保持同步）
    function makeDerivedPropsSelector() {
      ...
      return function selectDerivedProps(
        ...
        // 执行的是pureFinalPropsSelector(nextState, nextOwnProps) 函数 《selectorFactory.js》
        // 生成的是新的props (connect的)
    ...
    class Connect extends OuterBaseComponent {
      ...
      render() {
        ...
        // ContextToUse.Consumer  一个可以订阅 context 变化的 React 组件
        return (
